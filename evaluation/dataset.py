...
class LmdbDataset(Dataset):
    def __init__(self, root, opt, mode="train"):
        ...
        with self.env.begin(write=False) as txn:
            ...
            if mode == "train":
                if self.opt.part == "label_10":
                    self.nSamples = self.nSamples // 10
                    print("nSamples // 10")
                elif self.opt.part == "label_1":
                    self.nSamples = self.nSamples // 100
                    print("nSamples // 100")
...
class Text_augment(object):
    ...
    def __init__(self, opt):
        self.opt = opt
        self.Augment = iaa.Sequential([iaa.SomeOf((1, 4),
    ...
    def __call__(self, image):
        image = image.resize((self.opt.imgW, self.opt.imgH), PIL.Image.BICUBIC)
